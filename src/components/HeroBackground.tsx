...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Mix colors based on the red channel intensity of the generated pattern
    // This simple average was verified to work
    float intensity = (buf[0].x + buf[0].y + buf[0].z) / 3.0;
    
    // Mix using user colors
    vec3 col = mix(uColor1, uColor2, intensity);
    return vec4(col, 1.0);
  }
  
  void main() {
    // Stable UV calculation to avoid stretching/shifting when screen resizes (e.g. mobile scroll)
    vec2 resolution = iResolution;
    // Scale UVs to be -1 to 1 based on the MINIMUM dimension
    // This ensures zooming/cropping instead of stretching
    vec2 uv = (vUv - 0.5) * resolution / min(resolution.x, resolution.y) * 2.0;
...
function ShaderPlane({ color1, color2 }: ShaderPlaneProps) {
    ...
    if (materialRef.current && color1) materialRef.current.uColor1.set(color1);
    if (materialRef.current && color2) materialRef.current.uColor2.set(color2);

    return (
        <mesh ref={meshRef} position={[0, 0, 0]} scale={[viewport.width, viewport.height, 1]}>
            <planeGeometry args={[1, 1]} />
            {/* @ts-ignore - Custom shader material */}
            <brainMaterial ref={materialRef} side={THREE.DoubleSide} />
        </mesh>
    );
}

// Add strict type for custom element
...
const HeroBackground = ({
    ...
    return (
        <div
            ref={canvasRef}
            className={`absolute top-0 h-full pointer-events-none z-0 bg-white ${className}`}
            style={{
                left: 'calc(50% - 50vw)',
                width: '100vw'
            }}
        ...
        >
            <Canvas
                ...
                gl={{ antialias: true, alpha: true }} // alpha: true to support transparent background
                dpr={[1, 2]}
